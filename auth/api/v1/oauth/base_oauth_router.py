...
class OAuthRouter:
    ...
    def __init__(
        self, prefix: str, tags: list[str], oauth_service_class: Type[BaseOAuthService]
    ):
    ...
    def _setup_routes(self):
        ...
        async def callback(
            ...
            try:
                ...
                tokens = await oauth_service.authenticate(
                    code=code, state=state, device_id=device_id, metadata=metadata
                )
        ...
        @self.router.delete(
            "/unlink/{social_id}", status_code=status.HTTP_204_NO_CONTENT
        )
        async def unlink_account(
            ...
            try:
                ...
                await oauth_service.auth_service.unlink_social_account(
                    user_id=current_user["id"], provider=provider, social_id=social_id
                )
        ...
        @self.router.post("/refresh/{social_id}", response_model=SocialAccountResponse)
        async def refresh_token(
            ...
            try:
                ...
                updated_account = (
                    await oauth_service.auth_service.social_repository.update(
                        social_account.id,
                        {
...
vk_router = OAuthRouter(
    prefix="/vk", tags=["vk_oauth"], oauth_service_class=VKOAuthService
).router


yandex_router = OAuthRouter(
    prefix="/yandex", tags=["yandex_oauth"], oauth_service_class=YandexOAuthService
).router
