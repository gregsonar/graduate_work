...
class VKOAuthService(BaseOAuthService):
    ...
    async def get_user_info(self, access_token: str) -> Dict[str, Any]:
        ...
        async with await self.get_http_session() as session:
            async with session.post(self.settings.user_info_url, data=params, ssl=False) as response:
    ...
    async def process_user_info(self, user_info: Dict[str, Any], token_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def revoke_access(self, access_token: str) -> bool:
        ...
        async with await self.get_http_session() as session:
            async with session.post('https://id.vk.com/oauth2/revoke', data=params, ssl=False) as response:
                if response.status != 200:
                    return False
