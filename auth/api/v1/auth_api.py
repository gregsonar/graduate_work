...
async def get_auth_service(
    session: AsyncSession = Depends(get_session), redis_cli: Redis = Depends(get_redis)
) -> AuthService:
...
@router.post(
    "/logout", response_model=LogoutResponse, dependencies=[Depends(oauth2_scheme)]
)
async def logout(
...
async def register(
    auth_request: UserCreate, auth_service: AuthService = Depends(get_auth_service)
) -> TokenResponse:
    try:
...
@router.get("/access_logs", dependencies=[Depends(oauth2_scheme)])
