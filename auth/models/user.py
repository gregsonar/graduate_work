...
class User(Base, TimestampMixin):
    ...
    id: Mapped[uuid.UUID] = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, unique=True,
                                   nullable=False)
    ...
    def __init__(self, username: str, email: Optional[str] = None,
                 password: Optional[str] = None, is_superuser: bool = False,
    ...
    def add_social_account(self, provider: SocialProvider, social_id: str,
                           is_primary: bool = False, **social_data) -> UserSocialAccount:
    ...
    def remove_social_account(self, provider: SocialProvider) -> None:
        ...
        account = next((acc for acc in self.social_accounts if acc.provider == provider), None)
        if account:
            if account.is_primary and not self.password_hash:
                raise ValueError("Cannot remove primary authentication method without setting a password first")
            self.social_accounts.remove(account)

    def get_social_account(self, provider: SocialProvider) -> Optional[UserSocialAccount]:
