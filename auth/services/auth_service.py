...
class AuthService(UserRepository):
    ...
    async def register_user(
        ...
        try:
            event = UserCreatedEvent(user_id=new_user.id, email=new_user.email)
    ...
    async def change_password(
        ...
        try:
            ...
            raise HTTPException(
                HTTP_500_INTERNAL_SERVER_ERROR, detail="Unexpected error"
            )

    @circuit_protected
    async def authenticate_user(
        self, username: str, password: str, user_creds: dict = Optional[dict]
    ) -> Dict[str, Any]:
    ...
    async def logout_user(self, access_token: str, refresh_token: str) -> bool:
        if await self.auth.is_token_blacklisted(
            refresh_token
        ) or await self.auth.is_token_blacklisted(access_token):
            return True
    ...
    async def log_access(
        ...
        await access_log_repo.create_log(
            user_id=user_id, ip_address=ip_address, user_agent=user_agent
        )

    async def get_or_create_user(
        self, email: str, provider: str, username: str
    ) -> User:
        user = None
        try:
            ...
            if not user:
                ...
                user = await self.create({"username": username, "email": email})
    ...
    async def authenticate_social_user(
        ...
        try:
            ...
            if social_account:
                ...
                user = await self.get_user_by_social_identifiers(
            ...
            # Создаем токены
            access_token, refresh_token = await self.auth.create_tokens_for_user(
                user.id
            )
            return {"access_token": access_token, "refresh_token": refresh_token}
    ...
    async def get_user_by_social_identifiers(
        self, email: Optional[str], username: str, provider: SocialProvider
    ) -> Optional[User]:
    ...
    async def link_social_account(
        ...
        try:
            ...
            existing_account = (
                await self.social_repository.get_by_provider_and_social_id(
                    provider, social_id
                )
            )
            ...
            if existing_account:
                if existing_account.user_id == user_id:
                    ...
                    return await self.social_repository.update(
                        existing_account.id, social_data
                    )
                else:
                    raise HTTPException(
    ...
    async def unlink_social_account(
        self, user_id: UUID, provider: SocialProvider, social_id: str
    ) -> None:
