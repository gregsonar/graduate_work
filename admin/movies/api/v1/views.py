...
class MoviesApiMixin:
    ...
    def get_queryset(self):
        return FilmWork.objects.prefetch_related(
            'genres',
            Prefetch('personfilmwork_set', queryset=PersonFilmWork.objects.select_related('person'))
        )
...
class MovieSearch(APIView):
    def get(self, request):
        ...
        if query:
            q = MultiMatch(query=query, fields=['actors_names', 'writers_names', 'title', 'description', 'genres'],
                           fuzziness='auto')
            s = s.query(q)
        elif actor:
