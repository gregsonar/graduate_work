...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not username or not email or not password:
            self.stdout.write(
                self.style.ERROR(
                    "Environment variables for superuser are not set properly."
                )
            )
            return
        ...
        try:
            ...
            if response.status_code == 200:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Superuser {username} registered successfully in auth service."
                    )
                )
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Superuser {username} created successfully in Django."
                    )
                )
            else:
                self.stdout.write(
                    self.style.ERROR(
                        f"Failed to register superuser in auth service: {response.text}"
                    )
                )
